...
try{
 ...
 /*  Works but works better with fetchAll() 
    $result = $stmt->fetch(PDO::FETCH_ASSOC); //fetch - only get on row/record
...
catch(PDOException $e){ //catch bllock
...
.table div {
 ...
 <?php foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row){ 
   ...
   <div class="table">
    ...
    <!-- Links that Helpped: https://codepen.io/markheath/pen/ZEKwymp?editors=0100 -->
</html>