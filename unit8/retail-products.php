...
Get data from a database
    ...
    connect to database  - SQL Select
        prepare statement
...
<!--Hard Code-->
 ...
 <!--Single row/record-->       
        <div class="productBlock">
            ...
            <!-- The productStatus element should only be displayed if there is product_status data in the record -->
            <?php 
            //if (statues is NOT equal to empty then "")
                if($result['product_status']!=""){ //display  the element on page with the statues information 
                    //  if status != "" - DO show (false)
                ?>
            <p class="productStatus">New Item</p> 

            <?php
                }   //if status == "" - Do NOT show (true)
            ?>
...
<!-- foreach - Array -->
<?php 
    foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $result){ //array
?>
        <div class="productBlock">
            <div class="productImage">
                <image src="productImages/<?php echo $result['product_image']; ?>">
        ...
        <!-- The productStatus element should only be displayed if there is product_status data in the record -->
            <?php 
                //if (status is present) then display element on page with the statues information
                if($result['product_status']!=""){  // != means -->not equal to empty
